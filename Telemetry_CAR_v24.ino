...
float voltmin = 6.5;              // Tension minimale de la batterie
...
const int RPMLIMIT = 17000;       // Limiteur RPM
uint16_t LIMITEDRPM;
uint16_t ESCLIMITED;
...
// Commandes a distance, voir plus tard
};
...
RECEIVE_DATA_STRUCTURE COMM;
SEND_DATA_STRUCTURE TELEM;
...
void setup()
{
  ...
  esc.writeMicroseconds(NEUTRAL_THR);
  Serial.begin(9600);
  ETin.begin(details(COMM), &Serial);
  ETout.begin(details(TELEM), &Serial);
  Wire.begin();
  u8x8.begin();
  u8x8.clear();
